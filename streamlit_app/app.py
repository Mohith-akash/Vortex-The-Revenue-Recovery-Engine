...
from data_generator import generate_sample_data, PRODUCTS, ARCHETYPES
from ai_recovery import AIRecoveryEngine, RecoveryContext, create_recovery_context_from_event
from semantic_search import SemanticSearchEngine

st.set_page_config(page_title="Vortex", page_icon="⚡", layout="wide", initial_sidebar_state="collapsed")
...
def render_dashboard():
    ...
    st.caption(f"{len(df)} events • {len(df[df['event_type'] == 'cart_abandoned'])} abandoned • {len(df[df['event_type'] == 'checkout_success'])} orders")
    ...
    if len(abandoned) > 0:
        ...
        for _, row in abandoned.iterrows():
            ...
            activity.append({
                ...
                'Product': row['product_name'][:20] + '...' if len(row['product_name']) > 20 else row['product_name'],
    ...
    with col1:
        data = df[df['event_type'] == 'checkout_success'].groupby('geo_region')['cart_total'].sum().reset_index()
        top_region = data.loc[data['cart_total'].idxmax(), 'geo_region'] if len(data) > 0 else 'N/A'
        top_pct = (data['cart_total'].max() / data['cart_total'].sum() * 100) if len(data) > 0 else 0
        fig = go.Figure(go.Bar(
            x=data['geo_region'], y=data['cart_total'],
            marker=dict(color=data['cart_total'], colorscale=[[0, '#7c3aed'], [0.5, '#a855f7'], [1, '#ec4899']])
        ))
        fig.update_layout(**chart_theme(), height=250, showlegend=False, title='🌍 Revenue by Region')
        st.plotly_chart(fig)
    ...
    with col3:
        data = df[df['event_type'] == 'cart_abandoned'].groupby('user_archetype')['cart_total'].sum().reset_index()
        ...
        fig = go.Figure(go.Bar(
            x=data['user_archetype'], y=data['cart_total'],
            marker=dict(color=['#f87171', '#fb923c', '#fbbf24', '#a3e635', '#4ade80'])
        ))
        fig.update_layout(**chart_theme(), height=250, showlegend=False, title='🎯 Lost by Archetype')
        st.plotly_chart(fig)
        st.caption(f"**Highest loss:** {worst} (${worst_val:,.0f})")
    
    with col4:
        data = df[df['event_type'] == 'checkout_success'].groupby('device')['cart_total'].sum().reset_index()
        mobile = data[data['device'] == 'mobile']['cart_total'].sum() if 'mobile' in data['device'].values else 0
        desktop = data[data['device'] == 'desktop']['cart_total'].sum() if 'desktop' in data['device'].values else 0
        mobile_pct = (mobile / (mobile + desktop) * 100) if (mobile + desktop) > 0 else 0
        fig = go.Figure(go.Bar(
            x=data['device'], y=data['cart_total'],
            marker=dict(color=['#7c3aed', '#a855f7', '#c084fc'])
        ))
        fig.update_layout(**chart_theme(), height=250, showlegend=False, title='📱 Revenue by Device')
        st.plotly_chart(fig)
    ...
    fig.add_trace(go.Scatter(
        x=hours[:24], y=hourly_pattern[:24],
        mode='lines+markers',
        name='Historical',
    ...
    col3.metric("🎯 Recoverable", f"${forecast_total * (df['cart_total'].mean() or 150) * 0.32:,.0f}")
...
def render_recovery_analytics():
    ...
    total_revenue = successful['cart_total'].sum()
    ...
    timing_data = pd.DataFrame({
        'Timing': ['5 min', '30 min', '1 hr', '3 hr', '6 hr', '12 hr', '24 hr', '48 hr', 'None'],
    ...
    colors = ['#10b981', '#22c55e', '#84cc16', '#eab308', '#f59e0b', '#f97316', '#ef4444', '#dc2626', '#991b1b']
    
    col1, col2 = st.columns(2)
    
    with col1:
        fig = go.Figure(go.Bar(
            x=timing_data['Timing'], y=timing_data['Revenue'],
            marker=dict(color=colors),
            text=timing_data['Revenue'].apply(lambda x: f'${x:,.0f}'),
            textposition='outside'
        ))
        fig.update_layout(**chart_theme(), height=280, showlegend=False, title='💰 Revenue Recoverable')
        st.plotly_chart(fig)
    
    with col2:
        fig = go.Figure(go.Bar(
            x=timing_data['Timing'], y=timing_data['Carts'],
            marker=dict(color=colors),
            text=timing_data['Carts'],
            textposition='outside'
        ))
        fig.update_layout(**chart_theme(), height=280, showlegend=False, title='🛒 Carts Recoverable')
        st.plotly_chart(fig)
    ...
    if len(abandoned) > 0 and 'recovery_priority' in abandoned.columns:
        priority_data = abandoned.groupby('recovery_priority').agg(
            count=('cart_total', 'count'),
            value=('cart_total', 'sum')
        ).reset_index()
        ...
        with col2:
            ...
            fig.update_layout(**chart_theme(), height=250, showlegend=False, title='$ Value at Risk')
            st.plotly_chart(fig, )
    else:
    ...
    **What this shows:** Recovery rate comparison between SMS, Push Notifications, and Email at different timings.
    ...
    channel_data = pd.DataFrame({
        'Timing': ['5 min', '5 min', '5 min', '1 hour', '1 hour', '1 hour', '24 hours', '24 hours', '24 hours'],
        'Channel': ['SMS', 'Push', 'Email', 'SMS', 'Push', 'Email', 'SMS', 'Push', 'Email'],
        'Rate': [35, 30, 28, 22, 18, 26, 5, 3, 12]
    })
    
    fig = px.bar(channel_data, x='Timing', y='Rate', color='Channel', barmode='group',
    ...
    with col2:
        ...
        delay_rates = {'No messages': 2, '48 hours': 4, '24 hours': 7, '6 hours': 14, '1 hour': 24, '5 minutes': 32}
        ...
        st.metric("POTENTIAL DAILY GAIN", f"${improvement:,.0f}", f"↑ {((optimal_rate/max(1,current_rate))-1)*100:.0f}%")
...
def render_try_it():
    ...
    st.info("**⚡ Experience AI-powered cart recovery in action!** Fill in your details, add a product to cart, then abandon it to see the AI generate a personalized recovery message.")
    ...
    with col1:
        ...
        product = st.selectbox("📦 Product", PRODUCTS, format_func=lambda p: f"{p['name']} — ${p['price']:,.0f}")
    ...
    with col2:
        ...
        if abandon and name:
            ctx = RecoveryContext(
                customer_name=name, customer_archetype=archetype,
                product_name=product['name'], product_category=product['category'],
                cart_total=product['price'], abandonment_stage="cart",
                is_returning=False, discount_code="SAVE10" if product['price'] > 100 else None,
                utm_source="demo", session_duration=180, page_views=5,
            )
            with st.spinner("🧠 AI thinking..."):
                result = st.session_state.recovery_engine.generate_message(ctx)
            
            # Show result with styling
            source = "🤖 AI Generated (Cerebras)" if result['is_ai_generated'] else "📝 Template Fallback"
            ...
            logs = [
                ...
                f"👁️ Message opened!" if random.random() > 0.3 else "⏳ Awaiting open..."
            ]
            ...
            st.info(f"🛒 Added **{product['name']}** to cart. Now click **Abandon** to trigger recovery!")
...
def render_recovery_queue():
    ...
    st.info("**Priority-based recovery queue.** Click 'Generate' to create personalized AI recovery messages for each abandoned cart.")
    ...
    c1.metric("🔴 Critical", critical, f"${abandoned[abandoned['recovery_priority'] == 'critical']['cart_total'].sum():,.0f}")
    c2.metric("🟠 High", high)
    ...
    with col1:
        filt = st.multiselect("Filter Priority", ['critical', 'high', 'medium', 'low'], default=['critical', 'high'])
    with col2:
    ...
    for _, row in abandoned.head(10).iterrows():
        icon = {'critical': '🔴', 'high': '🟠', 'medium': '🟡', 'low': '🟢'}.get(row['recovery_priority'], '⚪')
        ...
        with col3:
            if st.button("🤖 Generate", key=f"g_{session_id}", ):
    ...
    **Priority Thresholds:** Critical ≥ 4 pts, High ≥ 2 pts, Medium ≥ 1 pt, Low = 0 pts
    ...
    if len(abandoned) > 0:
        ...
        st.info(f"**Example:** {top_cart['user_name']}'s cart → Priority **{top_cart.get('recovery_priority', 'unknown').upper()}** because: {' + '.join(reasons)}")


def render_search():
    st.markdown("## 🔍 Semantic Search")
    
    st.info("**AI-powered natural language search** using Voyage AI embeddings. Search for customer sessions using plain English descriptions.")
    ...
    query = st.text_input("🔎 Search Query", value=selected_sample or "", placeholder="Describe the sessions you're looking for...")
    ...
    if patterns.get('total_sessions', 0) > 0:
        conv_rate = patterns.get('by_outcome', {}).get('converted', 0) / patterns.get('total_sessions', 1) * 100
...
| **Interactive Demo** | Try the recovery system live |
    ...
    skills = {
        "Data Engineering": ["Event-driven architecture", "ETL/ELT pipelines", "Delta Lake", "DLT", "Streaming"],
        "Analytics Engineering": ["Medallion architecture", "Data modeling", "KPI design", "A/B metrics"],
        "AI/ML Engineering": ["LLM integration", "Prompt engineering", "Embeddings", "Semantic search"],
        "Software Engineering": ["Python", "API design", "CI/CD", "Git", "Modular code"],
        "Data Visualization": ["Plotly", "Streamlit", "Dashboard design", "UX/UI"],
        "Cloud & DevOps": ["Azure Event Hub", "Databricks", "GitHub Actions", "Environment management"]
    }
    ...
    libs = [
        "streamlit", "pandas", "plotly", "numpy", "faker",
        "cerebras-sdk", "voyageai", "python-dotenv", "azure-eventhub"
    ]
...
def render_ab_testing():
    ...
    st.info("**Message Experiment Dashboard** - Compare recovery message variants and identify winners with statistical significance.")
    ...
    avg_cart = abandoned['cart_total'].mean() if len(abandoned) > 0 else 150
    
    experiments = {
        "Urgency vs Friendly": {
            "variant_a": {"name": "🔥 Urgency", "message": "Your cart expires in 1 hour! Complete now →", "sends": int(total_abandoned * 0.5), "opens": vary(0.42), "clicks": vary(0.18), "conversions": vary(0.085)},
            "variant_b": {"name": "💚 Friendly", "message": "Hey! Your items are waiting. No rush! 😊", "sends": int(total_abandoned * 0.5), "opens": vary(0.38), "clicks": vary(0.15), "conversions": vary(0.072)},
        },
        "Discount vs Free Shipping": {
            "variant_a": {"name": "💰 10% Off", "message": f"Get 10% off your cart - use code SAVE10!", "sends": int(total_abandoned * 0.5), "opens": vary(0.45), "clicks": vary(0.22), "conversions": vary(0.095)},
            "variant_b": {"name": "🚚 Free Ship", "message": "Free shipping on your order - limited time!", "sends": int(total_abandoned * 0.5), "opens": vary(0.40), "clicks": vary(0.19), "conversions": vary(0.082)},
        },
        "SMS vs Email": {
            "variant_a": {"name": "📱 SMS", "message": "Quick checkout: tap to complete →", "sends": int(total_abandoned * 0.5), "opens": vary(0.68), "clicks": vary(0.28), "conversions": vary(0.110)},
            "variant_b": {"name": "📧 Email", "message": "Your cart is saved - complete your order", "sends": int(total_abandoned * 0.5), "opens": vary(0.35), "clicks": vary(0.12), "conversions": vary(0.065)},
        }
    }
    ...
    for col, (variant_key, variant) in zip([col1, col2], exp.items()):
        with col:
            opens = int(variant['sends'] * variant['opens'])
            clicks = int(variant['sends'] * variant['clicks'])
            conversions = int(variant['sends'] * variant['conversions'])
    ...
    with col3:
        winner_revenue = int(abandoned['cart_total'].sum() * (a['conversions'] if winner == "A" else b['conversions']))
        loser_revenue = int(abandoned['cart_total'].sum() * (b['conversions'] if winner == "A" else a['conversions']))
...
def main():
    t1, t2, t3, t4, t5, t6, t7 = st.tabs(["📊 Dashboard", "⏱️ Recovery Analytics", "🧪 A/B Testing", "🎮 Try It", "🤖 Queue", "🔍 Search", "🏗️ Architecture"])
    with t1: render_dashboard()
    with t2: render_recovery_analytics()
    with t3: render_ab_testing()
    with t4: render_try_it()
    with t5: render_recovery_queue()
    with t6: render_search()
    with t7: render_architecture()
